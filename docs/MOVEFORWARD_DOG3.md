...
`dog_move_forward.py` 是独立的 ROS 2 Python 速度控制节点，不属于
`dog_ego_planner` 规划包。它订阅里程计和局部路径，并可能向 `/vel_cmd` 发布
...
```bash
python3 /userdata/5_moveforward/dog_move_forward.py
```
...
python3 /userdata/5_moveforward/test_dog_move_forward.py -v
```
