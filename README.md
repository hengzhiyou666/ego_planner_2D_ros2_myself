...
`install/setup.bash`。dog3上会优先复用`/userdata/2_slam/setup_dog3_env.sh`，确保
SLAM、规划器和控制器使用同一Zenoh环境。它只启动规划器，不启动独立的
`7_moveforward`控制器，也不发布速度命令。可以先执行`./1_start_planner.sh --check`，
只检查环境而不启动节点。
...
| `odometry_topic` | `/relocalizing/map_frame/odometry` | 覆盖`topics.odom`；默认直连2号SLAM定位输出 |

`robot_launch.py` 作为一个主版本周期内的兼容入口保留，新部署应改用 `dog_ego_planner.launch.py`。Launch 文件不解析 YAML；`launch_rviz` 和 `use_sim_time` 均由 launch 参数显式控制。`odometry_topic`会覆盖参数文件中的`topics.odom`；默认直接订阅2号SLAM的定位输出，如需其他话题可显式传入`odometry_topic:=...`。

## ROS 接口

里程计、全局路径和局部路径固定使用绝对话题
`/relocalizing/map_frame/odometry`、`/map_frame/global_path`和
`/map_frame/local_path`。其他默认话题使用相对名称，可以跟随 ROS namespace。
多机器人部署时需要为每台机器人显式覆盖这些绝对话题参数。
...
| 参数 | 默认话题 | 类型 | 用途 |
|---|---|---|---|
| `topics.odom` | `/relocalizing/map_frame/odometry` | `nav_msgs/msg/Odometry` | 2号SLAM定位模式输出的地图位姿和速度 |
| `topics.global_path` | `/map_frame/global_path` | `nav_msgs/msg/Path` | `map_frame`中的自研全局参考路径 |
...
| 参数 | 默认话题 | 类型 | 用途 |
|---|---|---|---|
| `topics.local_path` | `/map_frame/local_path` | `nav_msgs/msg/Path` | 下游自研控制器使用的局部几何路径 |
...
节点默认在 `planning_frame=map_frame` 中规划。输入消息若不在该坐标系，必须能够通过 TF 转换；空 frame、无效四元数、过期/倒退的传感器时间戳以及超龄 TF 回退都会使节点进入停车状态。`/map_frame/local_path`、未完成全局路径、过滤点云和占据栅格等带坐标信息的规划输出都会使用该 `planning_frame`。生产部署应确认时间源、TF 树和消息时间戳一致。
...
定位话题`/relocalizing/map_frame/odometry`默认使用 reliable QoS，全局路径`/map_frame/global_path`默认使用 reliable + transient-local QoS；如上游发布者策略不同，可分别通过 `odometry_use_best_effort_qos` 和 `pct_path_match_nav2_qos` 调整。

地图坐标系中的全局路径和局部路径统一放在`/map_frame`话题层级下；其他自研接口继续使用独立名称，避免与厂家节点混用。
