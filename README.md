...
| `odometry_topic` | `lidar_location_now` | 覆盖`topics.odom`；`1_start_planner.sh`默认从0号适配层传入 |

`robot_launch.py` 作为一个主版本周期内的兼容入口保留，新部署应改用 `dog_ego_planner.launch.py`。Launch 文件不解析 YAML；`launch_rviz` 和 `use_sim_time` 均由 launch 参数显式控制。`odometry_topic`会覆盖参数文件中的`topics.odom`；使用`1_start_planner.sh`时自动取自0号适配层，直接调用launch并使用自定义参数文件时，如需其他话题应同时显式传入`odometry_topic:=...`。
...
| 参数 | 默认话题 | 类型 | 用途 |
|---|---|---|---|
| `topics.odom` | `/lidar_location_now` | `nav_msgs/msg/Odometry` | 自研 SLAM 输出的雷达当前地图位姿和速度 |
...
自研雷达定位话题`/lidar_location_now`默认使用 reliable QoS，全局路径`/pct_path_copy`默认使用 reliable + transient-local QoS；如上游发布者策略不同，可分别通过 `odometry_use_best_effort_qos` 和 `pct_path_match_nav2_qos` 调整。
