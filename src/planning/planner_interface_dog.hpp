...
public:
  ...
  void initGridMap(
    double x_size, double y_size, double resolution, const Eigen::Vector2d & origin,
...
private:
  ...
  bool refineTrajAlgo(
    ego_planner::UniformBspline & traj, std::vector<Eigen::Vector3d> & start_end_derivative,
  ...
  void reparamBspline(
    ego_planner::UniformBspline & bspline, std::vector<Eigen::Vector3d> & start_end_derivative,
    double ratio, Eigen::MatrixXd & ctrl_pts, double & dt, double & time_inc);
  bool trajectoryIsCollisionFree(const ego_planner::UniformBspline & trajectory) const;
